...
if sys.argv[2] == 'new':
    ...
    #fig.show()
    fig.savefig('calval/v31/calval_v31_%s_WC2_SOTWIS_GridSearch_XGB.png' % lvl,bbox_inches='tight')

    #now fit final forest on all dataset
    forest.fit(X,y)

    #save the fitted algorithm to avoid refitting everytime
    joblib.dump(forest,path+'/../saved_algorithms/kenya_ODA_v31_AGBpot_%s_WC2_SOTWIS_XGB.pkl' % lvl,compress = 1)
...
#figimp.show()
figimp.savefig('calval/v31/importances_v31_%s_WC2_SOTWIS_GridSearch_XGB.png' % lvl,bbox_inches='tight')

#save a netcdf file if needed
if sys.argv[3] =='savenc':
    import os

    fname = 'Kenya_ODA_v31_AGBpot_%s_WC2_SOTWIS_GridSearch_XGB.nc' % lvl
